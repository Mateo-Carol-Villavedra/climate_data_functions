...
The function intelligently handles different variable types during rescaling:
...
- Rescaled files exclude the term "processed" in their filename to distinguish them from the original high-resolution files
...
Rescale multiple ANUClimate variables using an AGCD template:
```
rescale_rasters(var_directories = c("path_to_clim_data/t_min", 
                                    "path_to_clim_data/t_max", 
                                    "path_to_clim_data/rain",
                                    "path_to_clim_data/srad",
                                    "path_to_clim_data/vp"),
                template_file = "path_to_clim_data/tmax/processed/tmax_2020_processed.nc")
```

<br/>

Rescale ANUClimate solar radiation to match SILO precipitation resolution:
```
rescale_rasters(var_directories = c("path_to_clim_data/srad"),
                template_file = "path_to_clim_data/radiation/processed/radiation_2019_processed.nc")
```
